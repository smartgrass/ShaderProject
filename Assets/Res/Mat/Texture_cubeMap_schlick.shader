Shader "Unlit/Texture_cubeMap_schlick"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag(outputData i):SV_TARGET{
				...
				float fresnel = saturate(_ReflectAmount + (1 - _ReflectAmount) * pow(1 - dot(worldNormal,viewDir),5));

				fixed3 color = specular+ ambient + lerp(diffuse,reflection,fresnel);
